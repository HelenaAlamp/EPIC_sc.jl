...
function space_charge_gl(bb::BunchedBeam, K, Nl, Nm, a, b, Np, dt)
    ...
    for i in 1:Nl
        for j in 1:Nm
            ...
            for k in 1:Np
                ...
                term1 .+= (al .* sin.(al .* rinx[k]) .* sin.(bm .* riny[k]) .* cos.(al .* rinx[k]) .* cos.(bm .* riny[k]))./gamma2lm[i,j]
                term2 .+= (bm .* sin.(al .* rinx[k]) .* sin.(bm .* riny[k]) .* cos.(al .* rinx[k]) .* cos.(bm .* riny[k]))./gamma2lm[i,j]
    ...
    bb.dist.px .-= (factor) .* term1 #aggiungo io
    bb.dist.py .-= (factor) .* term2 #aggiungo io
